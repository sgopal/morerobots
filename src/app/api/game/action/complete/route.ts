...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (activeActionError || !activeAction) {
      throw new Error(activeActionError?.message || 'Active action not found or does not belong to user');
    }

    if (activeAction.status === 'completed') {
      return NextResponse.json({ message: 'Action already completed' }, { status: 200 });
    }
    ...
    if (currentTime < endTime) {
      return NextResponse.json({ message: 'Action not yet complete' }, { status: 200 });
    }

    switch (activeAction.action_type) {
      case 'explore': {
        const { target_id: robotId, target_x: targetX, target_y: targetY, planet_id: planetId } = activeAction;

        // Update robot's location
        const { error: robotUpdateError } = await supabase
          .from('user_robots')
          .update({ current_x: targetX, current_y: targetY, current_planet_id: planetId })
          .eq('id', robotId)
          .eq('user_id', user.id);
        ...
        if (robotFetchError || !robotData || !robotData.robot_types) {
          throw new Error(robotFetchError?.message || 'Robot or robot type not found');
        }
        ...
        const { data: targetLocation, error: targetLocationError } = await supabase
          ...
          .select('id, has_aliens, alien_type_id, alien_quantity, alien_types(*)')
        ...
        if (targetLocation && targetLocation.has_aliens && targetLocation.alien_quantity > 0 && targetLocation.alien_types) {
          ...
          console.log(`Battle initiated at (${targetX}, ${targetY}) on planet ${planetId} against ${alienQuantity} ${targetLocation.alien_types.name}(s)!`);

          // Simple turn-based battle simulation
          while (currentRobotHealth > 0 && alienQuantity > 0) {
            // Robot attacks
            const robotDamage = Math.max(0, robotData.robot_types.ranged_attack - targetLocation.alien_types.ranged_defense); // Simple damage calc
            let damageToAlien = robotDamage;

            if (currentAlienShield > 0) {
              const shieldAbsorbed = Math.min(currentAlienShield, damageToAlien);
            ...
            console.log(`Robot attacks! Alien shield: ${currentAlienShield.toFixed(2)}, Alien health: ${currentAlienHealth.toFixed(2)}`);
            ...
            const alienDamage = Math.max(0, targetLocation.alien_types.melee_attack - robotData.robot_types.melee_defense); // Simple damage calc
            let damageToRobot = alienDamage;

            if (currentRobotShield > 0) {
              const shieldAbsorbed = Math.min(currentRobotShield, damageToRobot);
            ...
            console.log(`Alien attacks! Robot shield: ${currentRobotShield.toFixed(2)}, Robot health: ${currentRobotHealth.toFixed(2)}`);

            // Apply shield regeneration
            currentRobotShield = Math.min(robotData.robot_types.shield, currentRobotShield + robotData.robot_types.shield_regen_rate_per_second);
            currentAlienShield = Math.min(targetLocation.alien_types.shield, currentAlienShield + targetLocation.alien_types.shield_regen_rate_per_second);
            console.log(`Shields regenerated. Robot shield: ${currentRobotShield.toFixed(2)}, Alien shield: ${currentAlienShield.toFixed(2)}`);
          ...
          if (currentRobotHealth <= 0) {
            ...
            const { error: clearAliensError } = await supabase
              .from('planet_locations')
              .update({ has_aliens: false, alien_type_id: null, alien_quantity: 0 })
              .eq('id', targetLocation.id);
            if (clearAliensError) throw clearAliensError;
            console.log(`Aliens defeated at (${targetX}, ${targetY}) on planet ${planetId}`);
          }
        }
      ...
      case 'build_refinery': {
        const { target_id: buildingTypeId, planet_id: planetId, target_x: targetX, target_y: targetY } = activeAction;
        ...
        if (locationError || !locationData?.resource_id) {
          throw new Error(locationError?.message || 'Could not find resource at location for refinery');
        }
      ...
      default:
        console.warn(`Unhandled action type on completion: ${activeAction.action_type}`);
    }
