    ...
    switch (actionType) {
      case "explore": {
        if (!robotId || targetX === undefined || targetY === undefined) {
          return NextResponse.json(
            { error: "Missing parameters for explore action" },
            { status: 400 }
          );
        }
        ...
        if (robotError || !robot) {
          throw new Error(
            robotError?.message || "Robot not found or does not belong to user"
          );
        }
        ...
        return NextResponse.json(
          { message: "Exploration initiated", travelTimeSeconds },
          { status: 200 }
        );
      }
      ...
      case "build_refinery": {
        if (
          ...
          return NextResponse.json(
            { error: "Missing parameters for build_refinery action" },
            { status: 400 }
          );
        }
        ...
        if (
          userIridiumError ||
          (userIridium?.quantity || 0) < requiredIridium
        ) {
          return NextResponse.json(
            { error: "Insufficient Iridium to build refinery" },
            { status: 400 }
          );
        }
        ...
        if (buildingTypeError || !buildingType) {
          throw new Error(
            buildingTypeError?.message || "Building type not found"
          );
        }
        ...
        const buildEndTime = new Date(
          buildStartTime.getTime() + buildTimeSeconds * 1000
        );
        ...
        return NextResponse.json(
          { message: "Refinery construction initiated", buildTimeSeconds },
          { status: 200 }
        );
      }
      ...
      case "build_robot": {
        if (!currentPlanetId) {
          return NextResponse.json(
            { error: "Missing parameters for build_robot action" },
            { status: 400 }
          );
        }
        ...
        if (iridiumResourceError || !iridiumId) {
          return NextResponse.json(
            { error: "Iridium resource not found" },
            { status: 500 }
          );
        }
        ...
        const { data: basicRobotType, error: robotTypeFetchError } =
          await supabase
            .from("robot_types")
            .select("id")
        ...
        if (robotTypeFetchError || !basicRobotType) {
          return NextResponse.json(
            { error: "Basic Explorer Bot type not found" },
            { status: 500 }
          );
        }
        ...
        if (
          userIridiumError ||
          (userIridium?.quantity || 0) < requiredIridiumForRobot
        ) {
          return NextResponse.json(
            { error: "Insufficient Iridium to build robot" },
            { status: 400 }
          );
        }
        ...
        return NextResponse.json(
          { message: "Robot construction initiated", buildTimeSeconds },
          { status: 200 }
        );
      }
