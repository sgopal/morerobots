...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { data: existingPlanets, error: existingPlanetsError } = await supabase
      .from('user_planets')
      .select('id')
      .eq('user_id', user.id);
    ...
    const { data: newPlanet, error: planetError } = await supabase
      .from('planets')
      .insert({ name: `Player ${user.id.substring(0, 8)} Home Planet`, description: 'Your starting world.' })
      .select('id')
      .single();
    ...
    if (!robotTypeId) {
      const { data: newRobotType, error: newRobotTypeCreateError } = await supabase
        .from('robot_types')
        .insert({
    ...
    const { data: refineryBuildingType, error: buildingTypeError } = await supabase
      .from('building_types')
      .select('id')
    ...
    if (!refineryTypeId) {
      const { data: newRefineryType, error: newRefineryTypeError } = await supabase
        .from('building_types')
        .insert({ name: 'Refinery', description: 'Produces resources from mines.', base_crafting_time_seconds: 10 })
        .select('id')
        .single();
    ...
    return NextResponse.json({ message: 'Player initialized successfully' }, { status: 200 });
  } catch (error: any) {
