...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (userPlanets && userPlanets.length > 0) {
      return NextResponse.json({ error: 'User has already landed on a planet.' }, { status: 400 })
    }
    ...
    if (planets && planets.length > 0) {
      ...
      if (!anyPlanet || anyPlanet.length === 0) {
        return NextResponse.json({ error: 'No planets available to land on. Please seed the planets table.' }, { status: 500 })
      }
    ...
    const { data: initialResource, error: initialResourceError } = await supabase
      .from('resources')
      .select('id')
    ...
    if (!resourceId) {
      ...
      const { data: newResource, error: newResourceError } = await supabase
        .from('resources')
        .insert({ name: 'Basic Material', description: 'A fundamental crafting material.' })
        .select('id')
        .single()
    ...
    const { error: insertUserResourceError } = await supabase
      .from('user_resources')
      .insert({ user_id: user.id, resource_id: resourceId, planet_id: startingPlanetId, quantity: 500 })

    if (insertUserResourceError) throw insertUserResourceError

    // 4. Initialize starting vehicle (e.g., 'Scout Vehicle')
    const { data: initialVehicleType, error: initialVehicleTypeError } = await supabase
      .from('vehicle_types')
      .select('id')
    ...
    if (!vehicleTypeId) {
      const { data: newVehicleType, error: newVehicleTypeError } = await supabase
        .from('vehicle_types')
        .insert({
          name: 'Scout Vehicle',
    ...
    const { data: initialBuildingType, error: initialBuildingTypeError } = await supabase
      .from('building_types')
      .select('id')
    ...
    if (!buildingTypeId) {
      const { data: newBuildingType, error: newBuildingTypeError } = await supabase
        .from('building_types')
        .insert({
          name: 'Basic Refinery',
    ...
    return NextResponse.json({ message: 'Successfully landed on planet!', planet_id: startingPlanetId }, { status: 200 })
  } catch (error: any) {
