...
export default function PlanetMap({
  ...
  const [exploredCells, setExploredCells] = useState<Map<string, MapCell>>(
    new Map()
  );
  ...
  // Fetch explored locations from API
  const fetchMapData = useCallback(async () => {
    try {
      // Fetch explored locations
      const locationsResponse = await fetch(
        `/api/game/planets/${planetId}/locations`,
        {
      ...
      if (locationsResponse.ok) {
        ...
        locations.forEach((location: any) => {
          ...
          cellMap.set(key, {
            ...
            alienCount: location.alien_quantity,
            hasBuilding: false, // Will be populated by buildings data
          });
        });

        setExploredCells(cellMap);
      }

      // Fetch robots
      const robotsResponse = await fetch(
        `/api/game/planets/${planetId}/robots`,
        {
          headers: { Authorization: `Bearer ${accessToken}` },
        }
      );
      ...
      // Fetch buildings and update cells
  ...
  const renderMap = useCallback(() => {
    ...
    robots.forEach((robot) => {
      const robotScreenX = (robot.x - viewportCenter.x) * zoomLevel + halfWidth;
      const robotScreenY =
  ...
  }, [exploredCells, robots, viewportCenter, zoomLevel]);
  ...
  const handleRobotSelectionConfirm = useCallback(
    async (selectedRobots: string[]) => {
      ...
      try {
        const response = await fetch("/api/game/explore/start", {
          method: "POST",
          headers: {
        ...
        if (response.ok) {
          ...
          // Refresh map data to update robot positions
          await fetchMapData();
        } else {
          alert(`Failed to start exploration: ${data.error}`);
        }
      } catch (error) {
    ...
    [selectedTarget, accessToken, planetId, onLocationClick, fetchMapData]
  );
